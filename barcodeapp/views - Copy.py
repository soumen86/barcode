...
def invalidation(request,code):
    ...
    with connection.cursor() as cursor:
        update_q = "update saikat_rsvp.barcode_scan as t1 set food_service='invalid' where t1.barcode_num = '{0}'".format(code)
        print(update_q)
        cursor.execute(update_q)
        connection.commit()
        select_q = "select t1.id as TID, t2.id as food_update_id,t3.id as id,food_service,barcode_num,Name_Pri,Email,food_type,filename from saikat_rsvp.barcode_scan as t1 inner join saikat_rsvp.food_update as t2 inner join saikat_rsvp.rsvp_food as t3  where t1.food_update_id = t2.id and t2.rsvp_userid = t3.id and t1.barcode_num = '{0}'".format(code)
        cursor.execute(select_q)
        print(select_q)
        res_sel = dictfectchall(cursor)
        print(res_sel)
...
def fetchmember(request,ID):
    ...
    with connection.cursor() as cursor:
        select_q = "select t1.id as TID, t2.id as food_update_id,t3.id as id,food_service,barcode_num,Name_Pri,Email,food_type,filename from saikat_rsvp.barcode_scan as t1 inner join saikat_rsvp.food_update as t2 inner join saikat_rsvp.rsvp_food as t3  where t1.food_update_id = t2.id and t2.rsvp_userid = t3.id and t3.id = '{0}'".format(ID)
        cursor.execute(select_q)
        print(select_q)
        res_sel = dictfectchall(cursor)
        print(res_sel)
...
def scancode(request,code):
    ...
    with connection.cursor() as cursor:
        select_q = "select * from saikat_rsvp.barcode_scan as t1 where t1.barcode_num = '{0}' and food_service = 'invalid'".format(code)
        cursor.execute(select_q)
        print(select_q)
        ...
        if len(res_sel) > 0:
            details_info = []
            cursor.execute("select food_update_id,food_service,food_type,barcode_num from saikat_rsvp.barcode_scan as t1 where t1.barcode_num = '{0}'".format(code))
                        ...
                        for each_row in res:
                            ...
                            if temp == "Lunch" and each_row["lunch"] == 'N':
                                ...
                                while fod_head_count > 0:
                                    ...
                                    insertq = "INSERT INTO saikat_rsvp.barcode_scan (food_update_id, food_type, barcode_num,food_service, filename) VALUES ('{0}', 'lunch', '{1}','N', '{2}')".format(id,ean1, img_name)
                                ...
                                while fod_head_count_dinner > 0:
                                    ...
                                    if (vcount > 0):
                                        ...
                                        insertq = "INSERT INTO saikat_rsvp.barcode_scan (food_update_id, food_type, barcode_num,food_service, filename) VALUES ('{0}', 'dinner_veg', '{1}','N', '{2}')".format(id,ean1,img_name_d)
                                        vcount = vcount - 1
                                    elif(nvcount > 0):
                                        ...
                                        insertq = "INSERT INTO saikat_rsvp.barcode_scan (food_update_id, food_type, barcode_num,food_service,filename) VALUES ('{0}', 'dinner_nveg', '{1}','N', '{2}')".format(id,ean1,img_name_d)
                                        nvcount = nvcount - 1
                                    elif(kcount > 0):
                                        ean = EAN(f'{int(str(data_barcode))}', writer=ImageWriter())
                                        print("ean {0}".format(ean))
                                        ...
                                        insertq = "INSERT INTO saikat_rsvp.barcode_scan (food_update_id, food_type, barcode_num,food_service,filename) VALUES ('{0}', 'dinner_kid', '{1}','N', '{2}')".format(id,ean1,img_name_d)
                                        kcount = kcount - 1
                        ...
                        if res and len(res) > 0:
                            ...
                            for each_row in res:
                                ...
                                if temp == "Lunch" and each_row["lunch"] == 'N':
                                    ...
                                    subject = "[Saikat] Krishti 2023 | Food Coupon Barcodes - " + name_info
                                    text = "We are thrilled to share the digital Food Coupons for Krishti 2023 with you. These coupons are exclusively for the dinner food distribution event, specifically designed for adults food choice options." + "\n" \
+ "Whether you have a preference for vegetarian or non-vegetarian options, rest assured that these passes cover both choices." + "\n" \
+ " To ensure smooth management, we kindly request that you use a separate coupon for each adult mentioned in your RSVP. In most cases, the coupons are delivered to the email addresses provided during RSVP." + "\n" \
+ "If any family members have not received their coupons via email, please feel free to forward this email to them." + "\n" \
+ "If you have indicated any preferences for the kids' dinner, we kindly ask you to collect physical food coupons for your children from the registration desk." + "\n" \
+ "We sincerely appreciate your active participation in our coupon digitization process. Should you have any inquiries or require any assistance, please do not hesitate to reach out to the Registration desk at Krishti.." + "\n" \
+ "We eagerly anticipate providing you with a delightful experience at Krishti 2023." + "\n" + "Regards,\n" + "Saikat EC"
                                    ...
                                    while fod_head_count_dinner > 0:
                                        ...
                                        if (vcount > 0):
                                            ...
                                            kcount = kcount - 1
                                        fod_head_count_dinner = fod_head_count_dinner - 1
                                        files = os.path.join(settings.MEDIA_ROOT,img_name_d + ".png")
                                        file_list.append(files)
                                    ...
                                    #send_to = "ghosh.soumen86@gmail.com"
                                    subject = "[Saikat] Krishti 2023 | Food Coupon Barcodes - " + name_info
                                    text = "Dear " + each_row["Name_Pri"]  + "," + "\n" + "We are thrilled to share the digital Food Coupons for Krishti 2023 with you. These coupons are exclusively for the dinner food distribution event, specifically designed for adults food choice options." + "\n" \
+ "Whether you have a preference for vegetarian or non-vegetarian options, rest assured that these passes cover both choices." + "\n" + "\n" \
+ "To ensure smooth management, we kindly request that you use a separate coupon for each adult mentioned in your RSVP. In most cases, the coupons are delivered to the email addresses provided during RSVP." + "\n" \
+ "If any family members have not received their coupons via email, please feel free to forward this email to them." + "\n" + "\n" \
+ "If you have indicated any preferences for the kids' dinner, we kindly ask you to collect physical food coupons for your children from the registration desk." + "\n" \
...
def member(request):
     ...
     if request.method == 'POST':
            if 'search' in request.POST:
                ...
                select_q = "select * from saikat_rsvp.rsvp_food where Name_Pri like '%{0}%'".format(request.POST['name1_field'])
                print(select_q)
                with connection.cursor() as cursor:
                     cursor.execute(select_q)
