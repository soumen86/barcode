...
def send_mail(send_from, send_to, subject, text, files=None,
    ...
    print("send from = {0}".format(send_from))
    ...
    s.login(send_from, "qeve chah tavg vipr")
...
def footballclubs(request):
    ...
    with connection.cursor() as cursor:
        select_q = "select * from saikat_rsvp.krishti_rsvp_food"
        ...
        if len(res_sel) > 0:
            ...
            for each_row in res_sel:
                ...
                select_f = "select * from saikat_rsvp.krishti_food_update as t1 where t1.rsvp_userid = '{0}'".format(each_row["ID"])
                ...
                select_b = "select * from saikat_rsvp.krishti_food_update as t1 inner join saikat_rsvp.krishti_barcode_scan as t2 where t1.id = t2.food_update_id and t1.rsvp_userid = '{0}'".format(each_row["ID"])
...
def invalidation(request,code):
    ...
    with connection.cursor() as cursor:
        update_q = "update saikat_rsvp.krishti_barcode_scan as t1 set food_service='invalid' where t1.barcode_num = '{0}'".format(code)
        print(update_q)
        cursor.execute(update_q)
        connection.commit()
        select_q = "select t1.id as TID, t2.id as food_update_id,t3.id as id,food_service,barcode_num,Name_Pri,Email,food_type,filename from saikat_rsvp.krishti_barcode_scan as t1 inner join saikat_rsvp.krishti_food_update as t2 inner join saikat_rsvp.krishti_rsvp_food as t3  where t1.food_update_id = t2.id and t2.rsvp_userid = t3.id and t1.barcode_num = '{0}'".format(code)
        cursor.execute(select_q)
        print(select_q)
        res_sel = dictfectchall(cursor)
        print(res_sel)
...
def fetchmember(request,ID):
    ...
    with connection.cursor() as cursor:
        select_q = "select t1.id as TID, t2.id as food_update_id,t3.id as id,food_service,barcode_num,Name_Pri,Email,food_type,filename from saikat_rsvp.krishti_barcode_scan as t1 inner join saikat_rsvp.krishti_food_update as t2 inner join saikat_rsvp.krishti_rsvp_food as t3  where t1.food_update_id = t2.id and t2.rsvp_userid = t3.id and t3.id = '{0}'".format(ID)
        cursor.execute(select_q)
        print(select_q)
        res_sel = dictfectchall(cursor)
        print(res_sel)
...
def scancode(request,code):
    ...
    with connection.cursor() as cursor:
        select_q = "select * from saikat_rsvp.krishti_barcode_scan as t1 where t1.barcode_num = '{0}' and food_service = 'invalid'".format(code)
        cursor.execute(select_q)
        print(select_q)
        ...
        if len(res_sel) > 0:
            details_info = []
            cursor.execute("select food_update_id,food_service,food_type,barcode_num from saikat_rsvp.krishti_barcode_scan as t1 where t1.barcode_num = '{0}'".format(code))
            res = dictfectchall(cursor)
            print("done")
            for each_row1 in res:
                print(each_row1["food_update_id"])
                cursor.execute("select * from saikat_rsvp.krishti_rsvp_food as t1 inner join saikat_rsvp.krishti_food_update as t2 where t1.id = t2.rsvp_userid and t2.ID = '{0}'".format(each_row1["food_update_id"]))
            ...
            select_q = "select * from saikat_rsvp.krishti_barcode_scan as t1 where t1.barcode_num = '{0}' and food_service = 'Y'".format(code)
            cursor.execute(select_q)
            print(select_q)
            res_sel = cursor.fetchall()
            print(res_sel)
            ...
            if len(res_sel) > 0:
                ...
                update_q = "update saikat_rsvp.krishti_barcode_scan as t1 set food_service='Y' where t1.barcode_num = '{0}'".format(code)
                print(update_q)
                cursor.execute(update_q)
                connection.commit()
...
def viewcode(request,code):
    ...
    with connection.cursor() as cursor:
        select_q = "select * from saikat_rsvp.krishti_barcode_scan as t1 where t1.barcode_num = '{0}' and (checkin = 'completed' or food_service = 'AlreadyServed' or food_service = 'invalidcoupon')".format(code)
        cursor.execute(select_q)
        print(select_q)
        res_sel = cursor.fetchall()
        print(res_sel)
        ...
        if len(res_sel) > 0:
            details_info = []
            cursor.execute("select food_update_id,food_service,food_type,barcode_num, checkin from saikat_rsvp.krishti_barcode_scan as t1 where t1.barcode_num = '{0}'".format(code))
            res = dictfectchall(cursor)
            print("done")
            for each_row1 in res:
                print(each_row1["food_update_id"])
                ...
                for each_row in res1:
                    ...
                    details['email'] = each_row["Email"]
                    details['Lunch_all'] = each_row["Lunch_all"]
                    details['Snacks_veg'] = each_row["Snacks_veg"]
                    details['Snacks_nveg'] = each_row["Snacks_nveg"]
                    details['Dinner_veg'] = each_row["Dinner_veg"]
                    details['Dinner_nveg'] = each_row["Dinner_nveg"]
                    details['Dinner_kid'] = each_row["Dinner_kid"]
                    details['Volunteering'] = each_row["Volunteering"]
                    ...
                    details["barcode_num"] = each_row1["barcode_num"]
                    details["food_update_id"] = each_row1["food_update_id"]
                    details["checkin"] = each_row1["checkin"]
                    details_info.append(details)
                    #update_q = "update saikat_rsvp.krishti_barcode_scan as t1 set checkin='completed' where t1.barcode_num = '{0}'".format(code)
                    #print(update_q)
                    #cursor.execute(update_q)
                    #connection.commit()


            select_q = "select * from saikat_rsvp.krishti_rsvp_count"
            cursor.execute(select_q)
            res = dictfectchall(cursor)
            print("fetch rsvp count")
            total_lunch = 0
            total_dinnerveg = 0
            total_dinnernveg = 0
            total_dinnerkid = 0
            for each_row in res:
                total_lunch = total_lunch + int(each_row['Lunch'])
                total_dinnernveg = total_dinnernveg + int(each_row['Dinner_nveg'])
                total_dinnerveg = total_dinnerveg + int(each_row['Dinner_veg'])
                total_dinnerkid = total_dinnerkid + int(each_row['Dinner_kid'])

            return JsonResponse(data={'barcode_data': details_info,'lunch_count':total_lunch,'dinner_veg_count':total_dinnerveg,'dinner_nveg_count':total_dinnernveg,'dinner_kid_count':total_dinnerkid})


        else:
            select_q = "select * from saikat_rsvp.krishti_barcode_scan as t1 where t1.barcode_num = '{0}'".format(code)
            cursor.execute(select_q)
            print(select_q)
            res_sel = cursor.fetchall()
            print(res_sel)
            ...
            if len(res_sel) > 0:
                ...
                for each_row1 in res:
                    ...
                    for each_row in res1:
                        ...
                        details["barcode_num"] = each_row1["barcode_num"]
                        details["food_update_id"] = each_row1["food_update_id"]
                        details["checkin"] = "Done"
                        details_info.append(details)

                        update_q = "update saikat_rsvp.krishti_barcode_scan as t1 set checkin='completed' where t1.food_update_id = '{0}'".format(each_row1["food_update_id"])
                        print(update_q)
                        cursor.execute(update_q)
                        connection.commit()


                insertq = "INSERT INTO saikat_rsvp.krishti_rsvp_count (food_update_id,Name, Email, Lunch,Dinner_nveg, Dinner_veg, Dinner_kid) VALUES ('{0}','{1}', '{2}', '{3}','{4}', '{5}', '{6}')"\
                    .format(details["food_update_id"], details['name'], details['email'],details['Lunch_all'],details['Dinner_nveg'],details['Dinner_veg'], details['Dinner_kid'])
    ...
    if request.method == 'POST':
        ...
        if b_type == 'qrcode':
            ...
            if 'search' in request.POST:
                ...
                details_info = []
                with connection.cursor() as cursor:
                    cursor.execute("select * from saikat_rsvp.krishti_rsvp_food as t1 inner join saikat_rsvp.krishti_food_update as t2 where t1.id = t2.rsvp_userid and t1.Email = '{0}'".format(name_info))
                    res = dictfectchall(cursor)
                    print("done")
                    for each_row in res:
                        print(each_row["Name_Pri"])
                ...
                with connection.cursor() as cursor:
                    cursor.execute("select Name_Pri,Email,Lunch_all,Dinner_veg,Dinner_nveg,Dinner_kid,Volunteering,lunch,dinnerv,dinnernv,dinnerkid,t2.id as foodid from saikat_rsvp.krishti_rsvp_food as t1 inner join saikat_rsvp.krishti_food_update as t2 where t1.id = t2.rsvp_userid and t1.Email = '{0}'".format(name_info))
                    res = dictfectchall(cursor)
                    print("done")
                    if res and len(res) > 0:
                        ...
                        for each_row in res:
                            ...
                            if temp == "Lunch" and each_row["lunch"] == 'N':
                                update_q = "update saikat_rsvp.krishti_food_update as t1 inner join saikat_rsvp.krishti_rsvp_food as t2 set lunch='Y' where t2.id = t1.rsvp_userid and t2.Email = '{0}'".format(name_info)
                                print(update_q)
                                cursor.execute(update_q)
                                connection.commit()
                                ...
                                while fod_head_count > 0:
                                    ...
                                    insertq = "INSERT INTO saikat_rsvp.krishti_barcode_scan (food_update_id, food_type, barcode_num,food_service, filename) VALUES ('{0}', 'lunch', '{1}','N', '{2}')".format(id,ean1, img_name)
                                    print("insertq {0}".format(insertq))
                                    cursor.execute(insertq)
                                    connection.commit()
                            elif temp == "Dinner" and (each_row["dinnerv"] == 'N' and each_row["dinnernv"] == 'N' and each_row["dinnerkid"] == 'N'):
                                update_q = "update saikat_rsvp.krishti_food_update as t1 inner join saikat_rsvp.krishti_rsvp_food as t2 set dinnerv='Y',dinnernv='Y',dinnerkid='Y' where t2.id = t1.rsvp_userid and t2.Email = '{0}'".format(name_info)
                                print(update_q)
                                cursor.execute(update_q)
                                connection.commit()
                                ...
                                while fod_head_count_dinner > 0:
                                    ...
                                    if (vcount > 0):
                                        ...
                                        insertq = "INSERT INTO saikat_rsvp.krishti_barcode_scan (food_update_id, food_type, barcode_num,food_service, filename) VALUES ('{0}', 'dinner_veg', '{1}','N', '{2}')".format(id,ean1,img_name_d)
                                        vcount = vcount - 1
                                    elif(nvcount > 0):
                                        ...
                                        insertq = "INSERT INTO saikat_rsvp.krishti_barcode_scan (food_update_id, food_type, barcode_num,food_service,filename) VALUES ('{0}', 'dinner_nveg', '{1}','N', '{2}')".format(id,ean1,img_name_d)
                                        nvcount = nvcount - 1
                                    elif(kcount > 0):
                                        ean = EAN(f'{int(str(data_barcode))}', writer=ImageWriter())
                                        print("ean {0}".format(ean))
                                        ...
                                        insertq = "INSERT INTO saikat_rsvp.krishti_barcode_scan (food_update_id, food_type, barcode_num,food_service,filename) VALUES ('{0}', 'dinner_kid', '{1}','N', '{2}')".format(id,ean1,img_name_d)
                                        kcount = kcount - 1
                ...
                name_info_list = []
                with connection.cursor() as cursor:
                    #cursor.execute("select Name_Pri,Email from saikat_rsvp.krishti_rsvp_food where Email = 'ghosh.soumen86@gmail.com'")
                    cursor.execute("select Name_Pri, Email from saikat_rsvp.krishti_rsvp_food as t1 inner join saikat_rsvp.krishti_food_update as t2 where t2.lunch = 'N' and t1.id = t2.rsvp_userid")
                ...
                for name_info in name_info_list:
                    ...
                    with connection.cursor() as cursor:
                        cursor.execute("select Name_Pri,Email,Email2,Member_prev,Lunch_all,Dinner_veg,Dinner_nveg,Dinner_kid,Volunteering,lunch,dinnerv,dinnernv,dinnerkid,t2.id as foodid from saikat_rsvp.krishti_rsvp_food as t1 inner join saikat_rsvp.krishti_food_update as t2 where t1.id = t2.rsvp_userid and t1.Email = '{0}'".format(name_info))
                        ...
                        if res and len(res) > 0:
                            ...
                            for each_row in res:
                                ...
                                details['membership'] = each_row["Member_prev"]
                                details['Lunch_all'] = each_row["Lunch_all"]
                                details['Dinner_veg'] = each_row["Dinner_veg"]
                                details['Dinner_nveg'] = each_row["Dinner_nveg"]
                                details['Dinner_kid'] = each_row["Dinner_kid"]
                                ...
                                if temp == "Lunch" and each_row["lunch"] == 'N':
                                    ...
                                    while fod_head_count > 0:
                                        ...
                                        barcode_text  = details['email'].split("$")[0]  + '-' + temp  + '-' + details['membership'].split("(")[0]
                                        # original image
                                        barcode_image = code128.image(ean, height=100)
                                        # empty image for code and text - it needs margins for text
                                        w, h = barcode_image.size
                                ...
                                if temp == "Dinner" and (each_row["dinnerv"] == 'N' and each_row["dinnernv"] == 'N' and each_row["dinnerkid"] == 'N'):
                                    ...
                                    while fod_head_count_dinner > 0:
                                        ...
                                        if (vcount > 0):
                                            ...
                                            #img_name_d = each_row["Name_Pri"] + "_" + str(ean) + "_dinner_veg"
                                            img_name_d = each_row["Name_Pri"].replace(" ", "_") + "_" + str(ean) + "_dinner_veg.png"
                                            barcode_text  = details['email'] .split("$")[0] + '-' + temp + '-A veg-' + details['membership'].split("(")[0]
                                            barcode_image = code128.image(ean, height=100)
                                            # empty image for code and text - it needs margins for text
                                            w, h = barcode_image.size
                                            margin = 20
                                            ...
                                            img_name_d = each_row["Name_Pri"] + "_" + str(ean) + "_dinner_nveg.png"
                                            #barcode_text  = each_row["Name_Pri"] + '-' + temp + '-10-21-Day1-' + str(ean)
                                            barcode_text  = details['email'].split("$")[0]  + '-' + temp + '-A nonveg-'  + details['membership'].split("(")[0]
                                            barcode_image = code128.image(ean, height=100)
                                            # empty image for code and text - it needs margins for text
                                            w, h = barcode_image.size
                                            margin = 20
                                            ...
                                            new_h = h + (2*margin) # margin top and bottom
                                            # empty image with white border and new size
                                            new_image = Image.new('RGB', (new_w, new_h), (255, 165, 0))
                                            # put barcode on new image - with top margin
                                            new_image.paste(barcode_image, (0, margin))
                                            # object to draw text
                                            draw = ImageDraw.Draw(new_image)
                                            ...
                                            #img_name_d = each_row["Name_Pri"] + "_" + str(ean) + "_dinner_veg"
                                            img_name_d = each_row["Name_Pri"] + "_" + str(ean) + "_dinner_kid.png"
                                            barcode_text  = details['email'].split("$")[0]  + '-' + temp + '-K kid-'  + details['membership'].split("(")[0]
                                            barcode_image = code128.image(ean, height=100)
                                            # empty image for code and text - it needs margins for text
                                            w, h = barcode_image.size
                                            margin = 20
                                            ...
                                            new_h = h + (2*margin) # margin top and bottom
                                            # empty image with white border and new size
                                            new_image = Image.new('RGB', (new_w, new_h), (255, 248, 150))
                                            # put barcode on new image - with top margin
                                            new_image.paste(barcode_image, (0, margin))
                                            # object to draw text
                                            draw = ImageDraw.Draw(new_image)
                                    ...
                                    subject = "[Saikat] Krishti 2024 | Food Coupons | Dinner | " + details['membership'].split("(")[0] + " : " + name_info.split("$")[0] + ", " + details['email2']
                                    text = "Dear " + each_row["Name_Pri"]  + "," + "\n" + "We are excited to share the Digital Food Coupons for Krishti 2024. These coupons have been provided based on your Membership/Passes purchased and RSVP responses." + "\n" + "\n" \
+ "If you have any questions or concerns regarding the number or type of coupons you've received, we kindly ask you to visit our welcome desk during the event, where our team will be happy to assist you." + "\n" + "\n"  \
...
+ "\n"  \
+ "We are eagerly looking forward to providing you with a delightful experience at Krishti 2024." + "\n" + "\n" \
+ "Regards,\n" + "Saikat EC"
                                    send_from = "ghosh.soumen86@gmail.com"
                                    send_to = "secretary@saikat.org"
                                    #send_to = "soumen@saikat.org"
        ...
        with connection.cursor() as cursor:
            cursor.execute("select * from saikat_rsvp.krishti_rsvp_food as t1 inner join saikat_rsvp.krishti_food_update as t2 where t1.id = t2.rsvp_userid ")
            res = dictfectchall(cursor)
            print("done")
...
def dashboard(request):
     ...
     if request.method == 'POST':
            if 'fetch' in request.POST:
                ...
                food_not_serviced = 0
                with connection.cursor() as cursor:
                    cursor.execute("select * from saikat_rsvp.krishti_food_update as t1 inner join saikat_rsvp.krishti_barcode_scan as t2 where t1.id = t2.food_update_id and t2.food_type = '{0}'".format(temp))
                    res = dictfectchall(cursor)
                    print("done")
                    for each_row in res:
                        details = {}
                        cursor.execute("select * from saikat_rsvp.krishti_rsvp_food as t1 inner join saikat_rsvp.krishti_food_update as t2 where t1.id = t2.rsvp_userid and t2.id = '{0}'".format(each_row['food_update_id']))
                        res1 = dictfectchall(cursor)
                        if (temp == "Lunch"):
...
def member(request):
     ...
     if request.method == 'POST':
            if 'search' in request.POST:
                ...
                select_q = "select * from saikat_rsvp.krishti_rsvp_food where Name_Pri like '%{0}%'".format(request.POST['name1_field'])
                print(select_q)
                with connection.cursor() as cursor:
                     cursor.execute(select_q)
