...
def viewcode(request,code):
    ...
    with connection.cursor() as cursor:
        select_q = "select * from saikat_rsvp.sp_barcode_scan as t1 where t1.barcode_num = '{0}' and (checkin = 'completed' or food_service = 'AlreadyServed' or food_service = 'invalidcoupon')".format(code)
        cursor.execute(select_q)
        print(select_q)
        res_sel = cursor.fetchall()
        print(res_sel)
        ...
        if len(res_sel) > 0:
            details_info = []
            cursor.execute("select food_update_id,food_service,food_type,barcode_num, checkin from saikat_rsvp.sp_barcode_scan as t1 where t1.barcode_num = '{0}'".format(code))
            res = dictfectchall(cursor)
            print("done")
            for each_row1 in res:
                print(each_row1["food_update_id"])
                ...
                for each_row in res1:
                    ...
                    details_info.append(details)
                    #update_q = "update saikat_rsvp.sp_barcode_scan as t1 set checkin='completed' where t1.barcode_num = '{0}'".format(code)
                    #print(update_q)
                    #cursor.execute(update_q)
                    #connection.commit()


            select_q = "select * from saikat_rsvp.sp_rsvp_count"
            cursor.execute(select_q)
            res = dictfectchall(cursor)
            ...
            if len(res_sel) > 0:
                ...
                for each_row1 in res:
                    ...
                    for each_row in res1:
                        ...
                        update_q = "update saikat_rsvp.sp_barcode_scan as t1 set checkin='completed' where t1.food_update_id = '{0}'".format(each_row1["food_update_id"])
                        print(update_q)
                        cursor.execute(update_q)
                        connection.commit()


                insertq = "INSERT INTO saikat_rsvp.sp_rsvp_count (food_update_id,Name, Email, Lunch,Dinner_nveg, Dinner_veg, Dinner_kid) VALUES ('{0}','{1}', '{2}', '{3}','{4}', '{5}', '{6}')"\
        ...
        if b_type == 'qrcode':
            ...
            if 'search' in request.POST:
                ...
                with connection.cursor() as cursor:
                    ...
                    cursor.execute("select Name_Pri, Email from saikat_rsvp.sp_rsvp_food as t1 inner join saikat_rsvp.sp_food_update as t2 where t2.lunch = 'N' and t1.id = t2.rsvp_userid  limit 1")
                                ...
                                if temp == "Dinner" and (each_row["dinnerv"] == 'N' and each_row["dinnernv"] == 'N' and each_row["dinnerkid"] == 'N'):
                                    ...
                                    while fod_head_count_dinner > 0:
                                        ...
                                        if (vcount > 0):
                                            ...
                                            new_h = h + (2*margin) # margin top and bottom
                                            # empty image with white border and new size
                                            new_image = Image.new('RGB', (new_w, new_h), (160, 128, 96))
                                            # put barcode on new image - with top margin
                                            new_image.paste(barcode_image, (0, margin))
                                            # object to draw text
                                            draw = ImageDraw.Draw(new_image)
                                    ...
                                    subject = "[Saikat] Basanta Panchami 2024 | Food Coupons | Lunch And Dinner | " + details['membership'].split("(")[0] + " : " + name_info.split("$")[0] + ", " + details['email2']
                                    text = "Dear " + each_row["Name_Pri"]  + "," + "\n" + "We are excited to share the Digital Food Coupons for Basanta Panchami 2024. These coupons have been provided based on your Membership/Passes purchased and RSVP responses." + "\n" + "\n" \
...
+ "\n"  \
+ "We are eagerly looking forward to providing you with a delightful experience at Basanta Panchami 2024." + "\n" + "\n" \
+ "Regards,\n" + "Saikat EC"
